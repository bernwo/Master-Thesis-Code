...
import One_way_quantum_repeaters.cost_func_FT
...
min_L0 = 1e3;
max_L0 = 4e3;
c_search_array = 1:1000;
len_c = length(c_search_array);

len_L0 = 35;
L0_search_array = ones(len_L0,L_points,len_c).*99e3;
        ...
        for er_index = 1:len_er
            for c_index = 1:len_c
                [cf_vals(L_index,L0_index,c_index,er_index) , tree_arg(L_index,L0_index,c_index,er_index)] = ...
                    nonneg_min(cost_func_FT( Latt_val , tau_ph_val , L_vals(L_index) , L0_search_array(L0_index,L_index,c_index) , er_index , c_search_array(c_index) , constrained_treevectors(:,1) , constrained_treevectors(:,2) , constrained_treevectors(:,3) , eta_e_func , r0_inv_func , tau_cz_val ));
            end
        end
    end
end
...
for L_index = 1:L_points
    for er_index = 1:len_er
        [optimised_ptrans_vals(L_index,er_index),...
         ...
         optimised_rs_vals(L_index,er_index)] = One_way_quantum_repeaters.secretkeyrate_func_FT( L_vals(L_index) , optimised_L0_vals(L_index,er_index) , er_index , optimised_c_vals(L_index,er_index) , optimised_tree_vals(L_index,er_index,1) , optimised_tree_vals(L_index,er_index,2) , optimised_tree_vals(L_index,er_index,3) , eta_e_func , r0_inv_func , tau_cz_val , true );
    end
end
...
loglog(L_vals./1e3,optimised_rs_vals(:,3),'marker','d','color','red','linewidth',2,'linestyle','--'); hold on;
...
semilogx(L_vals./1e3,optimised_L0_vals(:,3)./1e3,'marker','d','color','red','linewidth',2,'linestyle','--'); hold on;
...
loglog(L_vals./1e3,optimised_c_vals(:,3),'marker','d','color','red','linewidth',2,'linestyle','--'); hold on;
...
grid on;
axis([L_min./1e3 L_max./1e3 min(c_search_array) max(c_search_array)])
...
semilogx(L_vals./1e3,get_inbetweenlinks(optimised_c_vals(:,3),L_vals',optimised_L0_vals(:,3)),'marker','d','color','red','linewidth',2,'linestyle','--'); hold on;
...
semilogx(L_vals./1e3,optimised_etae_vals(:,3),'marker','d','color','red','linewidth',2,'linestyle','--'); hold on;
...
semilogx(L_vals./1e3,optimised_ptrans_vals(:,3),'marker','d','color','red','linewidth',2,'linestyle','--'); hold on;

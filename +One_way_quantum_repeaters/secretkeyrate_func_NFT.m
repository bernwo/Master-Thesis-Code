function [ptrans,eta_e_eval,output] = secretkeyrate_func_NFT(L,L0,er_index,c,t0,t1,t2,eta_e_func,r0_inv_func,tau_cz,doPrint)
    ...
    if n < 1
        ...
        if (c >= 2)
            ...
            for k = 1:min(c,20) % maximum consider only k = 20. If more than that, then computation will be slow.
                ...
                eL_0loss_val = One_way_quantum_repeaters.eL_nFT_recursive2_mex(er_index,n,c-k);
        ...
        ec_noloss = One_way_quantum_repeaters.eL_nFT_recursive2_mex(er_index,n,c); % er_index here is set of indices i.e. {1,2,3,4,5}.
        
        rs_terms = ptrans1.*max(f(ec_noloss),0) + rs_terms;
        tau_spin_measure = 0; % seconds
        output = 1./(5.*r0_inv_func(t0,t1,t2) + 16*tau_cz + tau_spin_measure) .* rs_terms;
    end
end